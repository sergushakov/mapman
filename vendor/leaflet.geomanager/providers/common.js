    ...
    loadGoogleApi: function() {
        ...
        if ((typeof google === 'undefined') || (typeof google.maps==='undefined')) {
            $.getScript('https://www.google.com/jsapi', function() {
                google.load('maps', '3', {
                    other_params: 'libraries=places&key=AIzaSyAGo33r6UECbDAJV63G20ULh6RtyzKBkXc'
                    , callback: function() {
...
L.GeoManager.LayersFactory = {
    ...
    createSimpleIndentifyLayer: function(clickPromise) {
        return L.Class.extend({
            ...
            , _click: function (e) {
                var that = this;
                var tg = e.originalEvent.target;
                
                if (tg.getAttribute("data-dblclick") == null) {
                    tg.setAttribute("data-dblclick", 1);
                     setTimeout(function () {
                         if (tg.getAttribute("data-dblclick") == 1) {
                         ...
                         tg.removeAttribute("data-dblclick");
                     }, 300);
                 } else {
                     tg.removeAttribute("data-dblclick");
                 }
                
                // if (e.type === 'dblclick') {alert(1);};
...
L.GeoManager.GoogleIdentify = function (options) {
    ...
    $.when(L.GeoManager.APILoaders.loadGoogleApi()).done(function () {
        var identify = function (latlng) {
            ...
            var service = new google.maps.places.PlacesService($("#gmap")[0]);
            
            service.nearbySearch({ 'location': new google.maps.LatLng(latlng.lat, latlng.lng),radius: 1}, function(results,status){
                if (status == google.maps.places.PlacesServiceStatus.OK) {
                    ...
                    dfdid.resolve({
                        ...
                        // , bounds : bounds
                        , layer: new L.Circle(new L.LatLng(res.geometry.location.lat(), res.geometry.location.lng()),5)
                    });
                }
            });
