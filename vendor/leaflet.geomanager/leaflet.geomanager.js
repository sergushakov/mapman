...
L.GeoManager = L.Class.extend({
    ...
    , makeFun: function(options,action){
        var me = this,
            provider = me._getFunctionByName(options.providername);
        options.map = me.map;
        var layerPromise = provider(options);
        $.when(layerPromise).done(function(layer){
            if (action == "remove"){
                me._layers.removeLayer(me._layers.getLayerId(layer));
            } else {
                me._layers.addLayer(layer, true);
            }
        });
    }
    ...
    , _addLayer: function (l) {
        var me = this;
        console.log(me);
        me._layers.addLayer(l, true);
    }
